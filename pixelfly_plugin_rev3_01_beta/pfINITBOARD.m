...
% output : error_code [uint32] - zero on success, nonzero indicates failure,
...
if not(libisloaded('PCO_PF_SDK'))
 [notfound, warnings] = loadlibrary('pf_cam','PfcamMatlab.h' ...
              ,'addheader','PfcamExport.h' ...
              ,'alias','PCO_PF_SDK');
end

%libfunctionsview('PCO_PF_SDK');
ph_ptr=libpointer('voidPtrPtr');

board_num = int32(board_number);
% init camera
[error_code,board_handle] = calllib('PCO_PF_SDK', 'INITBOARD', board_num,ph_ptr);
end