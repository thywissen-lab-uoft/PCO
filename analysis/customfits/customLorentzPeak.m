...
% Lorentzian Function
% At (x-x0)=s --> y-->A/2 (s is the HWHM)
y = @(A,x0,s,x) A./(1+(x-x0).^2/s.^2);

% Assymetric Line Width (can replace s above)
% g(x=x0) = s0;
% g(x>>x0,a>0) = 0
% g(x<<x0,a>0) = 2*s0;
% g(x>>x0,a<0) = 2*s0
% g(x<<x0,a<0) = 0;
% Depending on sign of a, the linewidth goes from 0 (sharp) to 2*s0 (wide)
g = @(s0,a,x0,x) 2*s0./(1+exp(a*(x-x0)));
