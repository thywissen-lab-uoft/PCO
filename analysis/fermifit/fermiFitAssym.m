...
% Make the guess
Zg=ODfunc(xx,yy,gg(1),gg(2),gg(3),gg(4),gg(5),gg(6))+bg;
...
fitFermi_obj=fittype(@(A,Wx,Wy,Q,Xc,Yc,bg,X,Y) ODfunc(X,Y,A,Wx,Wy,Q,Xc,Yc)+bg,...
    'coefficients',{'A','Wx','Wy','Q','Xc','Yc','bg'},'independent',{'X','Y'});
...
fitopts.Lower=[0 W_gauss/5 W_gauss/5 -14 Xc-10 Yc-10 fitGauss.Fit.bg-.02];
fitopts.Upper=[A_max 5*W_gauss 5*W_gauss 20 Xc+10 Yc+10 fitGauss.Fit.bg+.02];
% fitopts.TolFun=1E-7;
...
% Acrew Errors
W_error = sqrt((0.5*range(fitFermi.ConfInt(:,2)))^2+(0.5*range(fitFermi.ConfInt(:,2)))^2);
Q_high = fitFermi.ConfInt(2,4);
Q_low = fitFermi.ConfInt(1,4);

% temperature value and error
T = m*(W*opts.PixelSize/opts.TOF)^2/kB;
T_error = 2*(T/W)*W_error;
...
% Find atom number
warning off
Natoms = real(1/crosssec*2*pi*(W*opts.PixelSize)^2*fout.A/6^(2/3)*(-polylog(3,-exp(fout.Q)))^(1/3));
...
function Z=ODfunc(X,Y,A,Wx,Wy,Q,xc,yc)
...
%     zz=exp(-(R2./(2*W.^2)));    % z=0 Gaussian distribution
    ...
    zz=exp(-(Y-yc).^2/(2*Wy^2)).*exp(-(X-xc).^2/(2*Wx^2));    % z=0 Gaussian distribution
