...
for kk=1:(ceil(length(atomdata)/pMax))    
    ...
    for ii=1:length(atomdataSUB)
        ...
        if isequal(style,'cut') && isequal(direction,'X')
           str = [str ' @ y = ' round(Yc) ' px']; 
        end
        if isequal(style,'cut') && isequal(direction,'X')
           str = [str ' @ x = ' round(Xc) ' px']; 
        end
        
        if doGauss
            if isequal(direction,'X')
               str = [str newline ...
                   'gauss {\bf x_c: }'  num2str(round(fitGauss.Xc)) ...
                    'px  ' ...
                    '{\bf \sigma_x: }' num2str(round(fitGauss.Xs)) ...
                    'px'];   
            else
                str = [str newline ...
                   'gauss {\bf y_c: }'  num2str(round(fitGauss.Yc)) ...
                    'px  ' ...
                    '{\bf \sigma_y: }' num2str(round(fitGauss.Ys)) ...
                    'px'];   
            end
        end
        ...
        if doErf
            if isequal(direction,'X')
               str = [str newline ...
                   'erf {\bf x_c: }'  num2str(round(fitGauss.Xc)) ...
                    'px  ' ...
                    '{\bf \sigma_x: }' num2str(round(fitGauss.Xs)) ...
                    'px'];   
            else
                str = [str newline ...
                   'erf {\bf y_c: }'  num2str(round(fitErf.Yc)) ...
                    'px  ' ...
                    '{\bf \sigma_y: }' num2str(round(fitErf.Ys)) ...
                    'px'];   
            end
        end
