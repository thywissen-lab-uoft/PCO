function [hF,output] = showAMSpec_RelNumber2(bm_am_spec_data,opts)
...
fmod = unique([bm_am_spec_data.Params.AM_spec_freq]);
uideal = freq2depth(fmod*1e-3);
...
str = ['f_{mod} = ' num2str(fmod*1e-3) ' kHz (' num2str(uideal) ' Er)'];
text(.01,.99,str,'units','normalized','verticalalignment','cap','fontsize',16);
%% Make Guesses
